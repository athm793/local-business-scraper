...
# Reskin theme defaults: monospace-forward console, squared controls, amber accents.
_THEME = ctk.ThemeManager.theme
_THEME["CTkFont"]["family"] = FONT_MONO
for _w in ("CTkButton", "CTkEntry", "CTkOptionMenu", "CTkComboBox"):
    _THEME[_w]["corner_radius"] = 6
_THEME["CTkCheckBox"]["corner_radius"]   = 4
_THEME["CTkCheckBox"]["fg_color"]        = AC_AMBER
_THEME["CTkCheckBox"]["hover_color"]     = BTN_GRN_H
_THEME["CTkCheckBox"]["checkmark_color"] = BRAND_INK
...
class App(ConfigMixin, LocationMixin, RunMixin, ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        sb.grid_rowconfigure(2, weight=1)   # scrollable body expands; brand + controls pinned

        # Brand (pinned top)
        brand = ctk.CTkFrame(sb, fg_color="transparent")
        brand.grid(row=0, column=0, sticky="ew", padx=16, pady=(16, 0))
        wm = ctk.CTkFrame(brand, fg_color="transparent")
        wm.pack(fill="x")
        ctk.CTkLabel(wm, text="maps_scraper",
                     font=ctk.CTkFont(size=15, weight="bold"), text_color=TX).pack(
            side="left")
        ctk.CTkLabel(wm, text="▋",
                     font=ctk.CTkFont(size=15, weight="bold"), text_color=BRAND).pack(
            side="left")
        ctk.CTkLabel(brand, text="google maps → csv",
                     font=ctk.CTkFont(size=10), text_color=TX_MUT, anchor="w").pack(
            fill="x", pady=(1, 0))

        ctk.CTkFrame(sb, height=1, fg_color=BD).grid(
            row=1, column=0, sticky="ew", pady=(8, 0))

        # ── Scrollable body: Search + Filters + Locations (never collapses) ──────
        ...
        self._kw_entry.pack(fill="x", padx=16, pady=(0, 8))
        ...
        self._depth_entry.pack(fill="x", padx=16, pady=(0, 8))
        ...
        self._workers_slider = ctk.CTkSlider(
            ...
            button_color=AC_AMBER, button_hover_color=BTN_GRN_H,
            progress_color=AC_AMBER,
            command=self._on_workers_change,
        )
        self._workers_slider.set(2)
        self._workers_slider.pack(fill="x", padx=16, pady=(0, 8))
        ...
        # ── Filters ───────────────────────────────────────────────────────────
        section_label(cfg_frame, "FILTERS · skip non-matching")
        ...
        self._filter_website = ctk.CTkOptionMenu(
            ...
            width=100, height=26, fg_color=BG_CARD, button_color=BD,
            dropdown_fg_color=BG_CARD, font=ctk.CTkFont(size=10))
        self._filter_website.pack(side="left", padx=(4, 8))
        ...
        ctk.CTkFrame(body, height=1, fg_color=BD).pack(fill="x", pady=(8, 0))
        ...
        ctk.CTkButton(
            ...
            font=ctk.CTkFont(size=11, weight="bold"), text_color=BRAND_INK,
        ...
        self._chip_frame = ctk.CTkFrame(loc_frame, fg_color="transparent")
        self._chip_frame.pack(fill="x", padx=16, pady=(0, 8))
        ...
        ctrl.grid(row=4, column=0, sticky="ew", padx=16, pady=(10, 14))
        ctrl.grid_columnconfigure(0, weight=1)

        self._start_btn = ctk.CTkButton(
            ctrl, text="Start scraping", height=42,
            fg_color=BTN_GRN, hover_color=BTN_GRN_H,
            font=ctk.CTkFont(size=13, weight="bold"), text_color=BRAND_INK,
            command=self._start,
        )
        ...
        self._stop_btn = ctk.CTkButton(
            ...
            fg_color="transparent", hover_color=BTN_RED_H,
            border_width=1, border_color=BTN_RED,
            font=ctk.CTkFont(size=12, weight="bold"), text_color=BTN_RED,
            state="disabled", command=self._stop,
        )
        self._stop_btn.grid(row=1, column=0, sticky="ew")

        self._out_label = ctk.CTkLabel(ctrl, text="",
                                        font=ctk.CTkFont(size=10), text_color=AC_AMBER,
    ...
    def _build_main(self):
        ...
        # ── Stats row (hairline-grid KPI strip: 1px gaps over a BD background) ───
        stats = ctk.CTkFrame(main, fg_color=BD, corner_radius=0, height=76)
        ...
        self._c_records.grid(row=0, column=0, padx=(0, 1), pady=(0, 1), sticky="nsew")

        self._c_rate = StatCard(stats, "Records / min", AC_GREEN)
        self._c_rate.grid(row=0, column=1, padx=(0, 1), pady=(0, 1), sticky="nsew")

        self._c_elapsed = StatCard(stats, "Elapsed", AC_AMBER)
        self._c_elapsed.grid(row=0, column=2, padx=(0, 1), pady=(0, 1), sticky="nsew")

        self._c_eta = StatCard(stats, "ETA", AC_PURPLE)
        self._c_eta.grid(row=0, column=3, padx=(0, 0), pady=(0, 1), sticky="nsew")
        ...
        ctk.CTkLabel(w_hdr, text="// WORKERS",
                     font=ctk.CTkFont(size=10, weight="bold"), text_color=AC_AMBER).pack(
        ...
        ctk.CTkLabel(log_hdr, text="// LIVE OUTPUT",
                     font=ctk.CTkFont(size=10, weight="bold"), text_color=AC_AMBER).grid(
        ...
        self._log_box.tag_config("ts",      foreground="#5a4c3d")
        self._log_box.tag_config("worker",  foreground=AC_AMBER)
        self._log_box.tag_config("success", foreground=AC_GREEN)
        self._log_box.tag_config("error",   foreground=AC_RED)
        self._log_box.tag_config("warn",    foreground="#fb8c3c")
